...
export const reservationRepository = {
  async create(data: CreateReservationData): Promise<ReservationRow> {
    ...
    return getDb().transaction(async (tx) => {
       ...
       * consumed it commit or roll back together. The guarded UPDATE in
       * redeemPromoCode means the last redemption goes to exactly one of two
       * concurrent bookings; the other fails here, before any charge exists,
       * rather than booking at a discount the code no longer had. */
      if (data.promoCodeId) {
